...
const CAREER_DATA_PATH = path.join(process.cwd(), "data", "career-data.json");
const RESUME_OUTPUT_PATH = path.join(process.cwd(), "content", "resume.md");
...
async function generate(): Promise<GenerationResult> {
  ...
  console.log(`   Thinking: adaptive (effort: max — Opus 4.6 exclusive, no constraints)`);
  ...
  // Load career data
  if (!fs.existsSync(CAREER_DATA_PATH)) {
    console.error(`❌ Career data not found: ${CAREER_DATA_PATH}\n`);
    console.error("   Run the ingestion step first: npm run ingest\n");
    process.exit(1);
  }

  const raw = fs.readFileSync(CAREER_DATA_PATH, "utf-8");
  ...
  console.log("   ⏳ Calling Claude API (this may take 30-90 seconds)...\n");
  ...
  const header = [
    ...
    `<!-- Generated: ${new Date().toISOString()} | Model: ${MODEL} | Tokens: ${response.usage.output_tokens} -->`,
    "",
  ].join("\n");
  ...
  // Write output
  const outputDir = path.dirname(RESUME_OUTPUT_PATH);
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }
  fs.writeFileSync(RESUME_OUTPUT_PATH, finalContent, "utf-8");
